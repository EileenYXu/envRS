...
sui = read.csv("abcdv5.1/substance-use/su_y_sui.csv") |> filter(
  ...
  select(src_subject_id, eventname, 
         # illicit drugs:
         tlfb_coc_use, tlfb_bsalts_use, tlfb_meth_use, tlfb_mdma_use, tlfb_ghb_use,
         tlfb_opi_use, tlfb_hall_use, tlfb_shrooms_use, tlfb_sniff_use, 
         # ever smoked tobacco
         tlfb_tob_puff)
...
subs_use = merge(sui, tlfb, by = c("src_subject_id", "eventname"), all.x=T) |> 
  select(src_subject_id, eventname, tlfb_cal_scr_alc_ud, tlfb_cal_scr_alc_max,
         su_tlfb_cal_scr_mj_days_yr, tlfb_cal_scr_num_events, illicit, 
         tlfb_tob_puff) |> mutate(
...
# make abuse and serious accident/injury trauma variables
ptsd = ptsd |> mutate(
...
ptsd = ptsd |> select(src_subject_id, eventname, ksads_ptsd_raw_760_p, 
                      ksads_ptsd_raw_770_p, ksads_ptsd_raw_766_p, sa, pa, ea,
                      serious_accident)
...
#### screentime ####
screentime = read.csv("abcdv5.1/novel-technologies/nt_y_st.csv") |> 
  filter(eventname=="2_year_follow_up_y_arm_1") |> mutate(
