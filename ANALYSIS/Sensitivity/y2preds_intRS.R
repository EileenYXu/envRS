...
# split training/test
index = createDataPartition(dat$cbcl_internalising, p = 0.2, list = F, times = 1)
...
for(i in 1:length(alphas)) {
  depfits[[paste0("alpha_", alphas[i])]] = cv.glmnet(
    x = train[,-c(1,2)], y = train[,2], foldid = foldid, type.measure = "deviance",
...
for (i in 1:length(depfits)) {
  trainfits[[paste0("alpha_", alphas[i])]] = assess.glmnet(
    depfits[[i]], newx = train[,-c(1,2)], newy = train[,2], s = "lambda.min")
}
...
## test data MSE = 0.62307   
assess.glmnet(depfits$alpha_0, newx = test[,-c(1,2)], newy = test[,2], s = "lambda.min")

## test data R^2 = 0.3123
