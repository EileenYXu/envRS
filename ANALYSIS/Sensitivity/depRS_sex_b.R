...
# Set up for EN ----
# binary predictors to be dummy coded as 0/1, birthsex recoded as M=0, F=1 below
...
xvars = c("src_subject_id", numpreds, binpreds, "birthsex")

# weight each ppt by % complete / m imputed datasets
misswts = rowMeans(!is.na(base_imp$data[,c(xvars, "cbcl_dsm5_depress")])) /50
...
dfs = dfs |> map(\(df) mutate(df,
                              ...
                              birthsex = case_when(birthsex=="M"~0, birthsex=="F"~1),
                              cbcl_dsm5_depress = scale(as.numeric(cbcl_dsm5_depress)),
                              misswt = misswts
)) |> futurize(seed=TRUE)

## stack training datasets ----
traindfs = dfs |> map(\(df) filter(df, src_subject_id %in% train_ids_sens)) |> 
  futurize(seed=TRUE)
train = reduce(traindfs, rbind) |> droplevels()
train_x = train |> select(all_of(xvars)) |> data.matrix()
train_y = train |> select(cbcl_dsm5_depress) |> data.matrix()
trainwts = train |> pull(misswt)

## test data ----
testdfs = dfs |> map(\(df) filter(df, src_subject_id %in% test_ids_sens)) |> 
  futurize(seed=TRUE)
...
foldid = sample(1:10, size = length(train_ids_sens), replace = TRUE) |> rep(times=50)
...
write.csv(res, file = "ANALYSIS/OUT/depRS_sex_base.csv", row.names = F)
