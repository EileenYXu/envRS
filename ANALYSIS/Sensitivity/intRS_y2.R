...
renv::load()
here::i_am("ANALYSIS/Sensitivity/intRS_y2.R")
...
misswts = rowMeans(!is.na(base_imp$data[,c(xvars, "cbcl_internalising_y2")])) /50
...
dfs = dfs |> map(\(df) mutate(df,
                              ...
                              cbcl_internalising_y2 = scale(as.numeric(cbcl_internalising_y2)),
                              misswt = misswts
)) |> futurize(seed=TRUE)
...
train_y = train |> select(cbcl_internalising_y2) |> data.matrix()
...
fitlist = testdfs |> 
  map(\(df) test_fit(df = df, outcome = "cbcl_internalising_y2",
                     coefs = coefs[-1]))
...
write.csv(res, file = "ANALYSIS/OUT/intRS_Y2.csv")
