...
renv::load()
here::i_am("ANALYSIS/Sensitivity/depRS_sex_y2.R")
...
misswts = rowMeans(!is.na(base_imp$data[,c(xvars, "cbcl_dsm5_depress_y2")])) /50
...
dfs = dfs |> map(\(df) mutate(df,
                              ...
                              cbcl_dsm5_depress_y2 = scale(as.numeric(cbcl_dsm5_depress_y2)),
                              misswt = misswts
)) |> futurize(seed=TRUE)
...
write.csv(res, file = "ANALYSIS/OUT/depRS_sex_Y2.csv")
