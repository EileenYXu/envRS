#########################################################################
## Predicting binary MDD status using depRS and PRS using EUR sumstats ##
#########################################################################
...
renv::load()
here::i_am("ANALYSIS/Sensitivity/depRS_PRS_eur.R")
...
# Read in data and scale depRS and PCs. PRS.sens already scaled across each ancestry
...
m1PRS.y = glmer(mdd.y ~ PRS.sens + PC1_AVG + PC2_AVG + PC3_AVG + PC4_AVG + PC5_AVG +
                  (1 | site_id_l), dat.y, family = binomial(link = "logit"),
                control = ctrl)

m2.y = glmer(mdd.y ~ depRS + PRS.sens + PC1_AVG + PC2_AVG + PC3_AVG + PC4_AVG + 
               PC5_AVG + (1 | site_id_l), dat.y, family = binomial(link = "logit"),
             control = ctrl)

m3.y = glmer(mdd.y ~ depRS*PRS.sens + PC1_AVG + PC2_AVG + PC3_AVG + PC4_AVG + 
               PC5_AVG + (1 | site_id_l), dat.y, family = binomial(link = "logit"),
             control = ctrl)
...
) |> select(mdd.y, m1depRS, m1PRS, m2, m3, depRS, PRS.sens)

preds = list("depRS" = dat.y$m1depRS, "PRS" = dat.y$m1PRS, 
             "depRS+PRS" = dat.y$m2, "depRS*PRS" = dat.y$m3)
...
res = list("fits" = auc, "depRS modcomp" = depRS_perf, "PRS modcomp" = PRS_perf)

## estimates
predictors = c("depRS", "PRS.sens", "depRS:PRS.sens")
...
# write results
openxlsx::write.xlsx(res, "ANALYSIS/OUT/sens_PRS_y.xlsx")
...
m1PRS.p = glmer(mdd.p ~ PRS.sens + PC1_AVG + PC2_AVG + PC3_AVG + PC4_AVG + PC5_AVG +
                  (1 | site_id_l), dat.p, family = binomial(link = "logit"),
                control = ctrl)

m2.p = glmer(mdd.p ~ depRS + PRS.sens + PC1_AVG + PC2_AVG + PC3_AVG + PC4_AVG + 
               PC5_AVG + (1 | site_id_l), dat.p, family = binomial(link = "logit"),
             control = ctrl)

m3.p = glmer(mdd.p ~ depRS*PRS.sens + PC1_AVG + PC2_AVG + PC3_AVG + PC4_AVG + 
               PC5_AVG + (1 | site_id_l), dat.p, family = binomial(link = "logit"),
             control = ctrl)
...
) |> select(mdd.p, m1depRS, m1PRS, m2, m3, depRS, PRS.sens)

preds = list("depRS" = dat.p$m1depRS, "PRS" = dat.p$m1PRS, 
             "depRS+PRS" = dat.p$m2, "depRS*PRS" = dat.p$m3)
...
# write results
openxlsx::write.xlsx(res, "ANALYSIS/OUT/sens_PRS_p.xlsx")