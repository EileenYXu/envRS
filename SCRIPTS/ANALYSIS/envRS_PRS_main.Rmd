...
For depRS-only analyses, there are `r nrow(filter(y2dat_y, mdd_lifetime==1))` lifetime MDD cases, `r nrow(filter(y2dat_y, mdd_present==1))` current MDD cases and `r nrow(filter(y2dat_y, any_lifetime==1))` cases with lifetime diagnoses of a depressive disorder (Major Depressive Disorder, Persistent Depressive Disorder (dysthymia), Depressive Disorder Unspecified). \
\
Of those with both envRS and PRS at Y2 (N = `r nrow(y2prs_y)`), there are `r nrow(filter(y2prs_y, mdd_lifetime==1))` lifetime MDD cases, `r nrow(filter(y2prs_y, mdd_present==1))` current MDD cases and `r nrow(filter(y2prs_y, any_lifetime==1))` cases with lifetime diagnoses of a depressive disorder (Major Depressive Disorder, Persistent Depressive Disorder (dysthymia), Depressive Disorder Unspecified). \
\

### depRS only

```{r}
depmod = glm(mdd_lifetime ~ depRS_b, y2dat_y, family = binomial(link = "logit"))
...
roc(y2dat_y$mdd_lifetime, depmod$fitted.values)
```
...
For depRS-only analyses, there are `r nrow(filter(y2dat_p, mdd_lifetime==1))` lifetime MDD cases, `r nrow(filter(y2dat_p, mdd_present==1))` current MDD cases and `r nrow(filter(y2dat_p, any_lifetime==1))` cases with lifetime diagnoses of a depressive disorder (Major Depressive Disorder, Persistent Depressive Disorder (dysthymia), Depressive Disorder Unspecified). \
\
Of those with both envRS and PRS at Y2 (N = `r nrow(y2prs_p)`), there are `r nrow(filter(y2prs_p, mdd_lifetime==1))` lifetime MDD cases, `r nrow(filter(y2prs_p, mdd_present==1))` current MDD cases and `r nrow(filter(y2prs_p, any_lifetime==1))` cases with lifetime diagnoses of a depressive disorder (Major Depressive Disorder, Persistent Depressive Disorder (dysthymia), Depressive Disorder Unspecified). \
\

### depRS only

```{r}
depmod = glm(mdd_lifetime ~ depRS_b, y2dat_p, family = binomial(link = "logit"))
sjPlot::tab_model(depmod, collapse.ci = T, show.se = T, show.stat = T)

performance_accuracy(depmod)|> as.data.frame() |> kable(digits = 3) |> kable_styling()

roc_depRS = roc(y2dat_p$mdd_lifetime, depmod$fitted.values)
```
