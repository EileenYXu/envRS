---
title: "depRS and PRS main analyses"
date: "`r Sys.Date()`"
output:
  html_document:
    code_folding: hide
...
PRS = bind_rows(mdd2025$afr, mdd2025$eas, mdd2025$amr_diverse, mdd2025$eur_diverse)
...
names(scores) = c("src_subject_id", "depRS", "PRS")
```

depRS was available for N=`r nrow(depRS)` unrelated participants.\
PRS was available for N=`r nrow(PRS)` participants.\

A total N=`r nrow(scores)` *unrelated* participants had complete depRS and PRS data.
...
dat.y = merge(scores, mddy$y2[,c("src_subject_id", "mdd_lifetime")], by = "src_subject_id")
...
m1PRS.y = glm(mdd_lifetime ~ PRS, dat.y, family = binomial(link = "logit"))

m2.y = glm(mdd_lifetime ~ depRS + PRS, 
         dat.y, family = binomial(link = "logit"))
m3.y = glm(mdd_lifetime ~ depRS*PRS,
         dat.y, family = binomial(link = "logit"))
```
...
```{r}
sjPlot::tab_model(mods.y, dv.labels = c("depRS", "PRS", "depRS+PRS", "depRS*PRS"), collapse.ci = T, show.se = T, show.stat = T)
```
...
m1PRS.p = glm(mdd_lifetime ~ PRS, dat.p, family = binomial(link = "logit"))

m2.p = glm(mdd_lifetime ~ depRS + PRS, 
         dat.p, family = binomial(link = "logit"))
m3.p = glm(mdd_lifetime ~ depRS*PRS,
         dat.p, family = binomial(link = "logit"))
```
...
```{r}
sjPlot::tab_model(mods.p, dv.labels = c("depRS", "PRS", "depRS+PRS", "depRS*PRS"), collapse.ci = T, show.se = T, show.stat = T)
```
...
```{r}
ggsave("C://Users/eilee/Desktop/PhD/Year 3/Drafts/envRS_paper/Figures/ROCs_main.png", ptch_plot, device = "png", create.dir = TRUE, width = 9, height = 4, units = "in")
```
