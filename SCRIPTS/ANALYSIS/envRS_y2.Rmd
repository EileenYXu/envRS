...
train$bullying_gender = train$bullying_victim * as.numeric(train$gender)

test = test |> mutate_at(numcols, ~ c(scale(.)) )
test$bullying_gender = test$bullying_victim * as.numeric(test$gender)

## remove columns that are not used for training data
preds = train |> select(-src_subject_id, -race_ethnicity, -cbcl_internalising, -cbcl_dsm5_depress, -birthsex) |> data.matrix()

## remove columns not used in test data and make interactions
test_preds = test |> select(-src_subject_id, -race_ethnicity, -cbcl_internalising, -cbcl_dsm5_depress, -birthsex) |> data.matrix()
...
saveRDS(depRS, "G://users/eileen/ABCD/ABCD_Environmental_Risk/ABCDv5.1/DATA/depRS_y2.rds")
```
...
saveRDS(intRS, "G://users/eileen/ABCD/ABCD_Environmental_Risk/ABCDv5.1/DATA/intRS_y2.rds")
```
...
write.csv(y2_coefs, "C://Users/eilee/Desktop/PhD/Year 3/Drafts/envRS_paper/y2_coefs.csv")
```

```{r fig.height=20, fig.width=10, fig.cap="Nonzero"}
## plot coefficients for depRS and intRS
coef_long = pivot_longer(y2_coefs[-1,], cols = c("depRS", "intRS"), names_to = "out", values_to = "beta.min")

y2_coefs_sig = coef_long |> filter(abs(round(beta.min, 3))!=0)

## pivot this back to wide
coefs_sig = y2_coefs_sig |> pivot_wider(names_from = out, values_from = beta.min)

write.csv(coefs_sig, "C://Users/eilee/Desktop/PhD/Year 3/Drafts/envRS_paper/y2_coefs_sig.csv")
```
...
ggsave(filename = "C://Users/eilee/Desktop/PhD/Year 3/Drafts/envRS_paper/y2_coefs_sig.svg", height = 200, width = 200, units = "mm")
```
...
coef_01 = y2_coefs_sig |> filter(abs(beta.min)>=0.1)
```
...
ggsave(filename = "C://Users/eilee/Desktop/PhD/Year 3/Drafts/envRS_paper/y2_01.svg", height = 140, width = 200, units = "mm")
```
...
train = readRDS("G://users/eileen/ABCD/ABCD_Environmental_Risk/ABCDv5.1/DATA/y2_traindat.rds") |> na.omit() |> filter(gender != "GNC") #6090 individuals
test = readRDS("G://users/eileen/ABCD/ABCD_Environmental_Risk/ABCDv5.1/DATA/y2_testdat.rds") |> na.omit() |> filter(gender != "GNC") #1499
