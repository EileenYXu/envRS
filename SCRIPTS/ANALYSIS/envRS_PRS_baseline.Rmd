---
title: "Baseline envRS and PRS for longitudinal outcomes"
output:
  html_document:
    toc: true
    toc_float:
...
Baseline envRS were trained on parent-reported CBCL subscales for internalizing (intRS) and depression (depRS) symptoms. Here I am testing associations between baseline envRS and longitudinal outcomes, plus the combination of envRS and MDD3 PRS.
...
envRS = envRS |> select(src_subject_id, cbcl_dep_b, cbcl_int_b, depRS_b, intRS_b)
...
ggplot(dat) +
  geom_density(aes(x=depRS_b), colour = 3) +
  geom_density(aes(x = scale(PRS)), colour = 4)
```


PRS data based on MDD3 GWAS (from Poppy) were available for `r nrow(PRS)` participants. At year 3, CBCL data were available for `r nrow(cbcl_y3)` participants. A total of `r nrow(s_dat)` participants have year 3 CBCL data, PRS and baseline envRS. \
\
...
```{r}
m1 = lm(dep ~ depRS_b, data = s_dat)
m2 = lm(dep ~ depRS_b + scale(PRS), data = s_dat)
m3 = lm(dep ~ depRS_b*scale(PRS), data = s_dat)
...
```{r}
dep_mod = lm(dep ~ depRS_b, e_dat)
comb_mod = lm(dep ~ depRS_b + intRS_b, e_dat)
interact = lm(dep ~ depRS_b*intRS_b, e_dat)
...
```{r}
m1 = lm(int ~ intRS_b, data = s_dat)
m2 = lm(int ~ intRS_b + scale(PRS), data = s_dat)
m3 = lm(int ~ intRS_b*scale(PRS), data = s_dat)
...
```{r}
int_mod = lm(int ~ intRS_b, e_dat)
comb_mod = lm(int ~ intRS_b + depRS_b, e_dat)
interact = lm(int ~ intRS_b*depRS_b, e_dat)
...
At year 4, CBCL data were available for `r nrow(cbcl_y4)` participants. A total `r nrow(s_dat)` participants have year 4 CBCL data, PRS and baseline envRS. \
\
Model comparisons are run for: \
\
m1 = cbcl ~ envRS \
...
Depression diagnoses (lifetime and current) from *youth-reported* KSADS are available at baseline (N=`r nrow(basedat)`) and year 2 (N=`r nrow(y2dat)`). \
...
\
Of those with both envRS and PRS at baseline (N = `r nrow(baseprs)`), there are `r nrow(filter(baseprs, mdd_lifetime==1))` lifetime MDD cases, `r nrow(filter(baseprs, mdd_present==1))` current MDD cases and `r nrow(filter(baseprs, any_lifetime==1))` cases with lifetime diagnoses of a depressive disorder (Major Depressive Disorder, Persistent Depressive Disorder (dysthymia), Depressive Disorder Unspecified). \
\
...
```{r}
m1 = glm(mdd_lifetime ~ depRS_b, baseprs, family = binomial(link = "logit"))
m2 = glm(mdd_lifetime ~ depRS_b + scale(PRS), baseprs, family = binomial(link = "logit"))
m3 = glm(mdd_lifetime ~ depRS_b*scale(PRS), baseprs, family = binomial(link = "logit"))
...
```{r}
m1 = glm(mdd_lifetime ~ intRS_b, baseprs, family = binomial(link = "logit"))
m2 = glm(mdd_lifetime ~ intRS_b + scale(PRS), baseprs, family = binomial(link = "logit"))
m3 = glm(mdd_lifetime ~ intRS_b*scale(PRS), baseprs, family = binomial(link = "logit"))
...
```{r}
depRS = glm(mdd_lifetime ~ depRS_b, basedat, family = binomial(link = "logit"))
intRS = glm(mdd_lifetime ~ intRS_b, basedat, family = binomial(link = "logit"))
combRS = glm(mdd_lifetime ~ depRS_b + intRS_b, basedat, family = binomial(link = "logit"))
interactRS = glm(mdd_lifetime ~ depRS_b*intRS_b, basedat, family = binomial(link = "logit"))
...
m2 = glm(mdd_lifetime ~ depRS_b + scale(PRS), y2prs, family = binomial(link = "logit"))
m3 = glm(mdd_lifetime ~ depRS_b*scale(PRS), y2prs, family = binomial(link = "logit"))
...
```{r}
m1 = glm(mdd_lifetime ~ intRS_b, y2prs, family = binomial(link = "logit"))
m2 = glm(mdd_lifetime ~ intRS_b + scale(PRS), y2prs, family = binomial(link = "logit"))
m3 = glm(mdd_lifetime ~ intRS_b*scale(PRS), y2prs, family = binomial(link = "logit"))
...
```{r}
depRS = glm(mdd_lifetime ~ depRS_b, y2dat, family = binomial(link = "logit"))
intRS = glm(mdd_lifetime ~ intRS_b, y2dat, family = binomial(link = "logit"))
combRS = glm(mdd_lifetime ~ depRS_b + intRS_b, y2dat, family = binomial(link = "logit"))
interactRS = glm(mdd_lifetime ~ depRS_b*intRS_b, y2dat, family = binomial(link = "logit"))
...
baseprs = merge(scores, mddp$base, by = "src_subject_id")
y2prs = merge(scores, mddp$y2, by = "src_subject_id")
```

Depression diagnoses (lifetime and current) from *parent-reported* KSADS are available at baseline (N=`r nrow(basedat)`) and year 2 (N=`r nrow(y2dat)`). \
