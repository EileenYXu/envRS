...
I had the (5-level) ethnicity variable in for mice imputation, but the imputed missing data is influenced by the other variables in the data (and thus entered into the model) and when it comes to longitudinal analysis it might make things overly complicated. Doing complete sample analysis instead.
...
```{r}
baseline[,-1] |> dfSummary(plain.ascii = F, style = "grid", valid.col = F, graph.col = T, varnumbers = F, tmp.img.dir = "/tmp", max.tbl.height = 500) |> print(method = "render")
```

### Training data:

```{r}
traindat[,-1] |> summarytools::dfSummary(plain.ascii = F, style = "grid", valid.col = F, graph.col = T, varnumbers = F, tmp.img.dir = "/tmp", max.tbl.height = 500) |> print(method = "render")
```

### Test data:

```{r}
testdat[,-1] |> summarytools::dfSummary(plain.ascii = F, style = "grid", valid.col = F, graph.col = T, varnumbers = F, tmp.img.dir = "/tmp", max.tbl.height = 500) |> print(method = "render")
```
