...
```{r}
train = readRDS("G://users/eileen/ABCD/ABCD_Environmental_Risk/ABCDv5.1/DATA/y2_traindat.rds") |> na.omit() |> filter(gender!="GNC")
test = readRDS("G://users/eileen/ABCD/ABCD_Environmental_Risk/ABCDv5.1/DATA/y2_testdat.rds") |> na.omit() |> filter(gender!="GNC")
...
80-20 training-test split. Training sample N=`r nrow(train)`, test sample N=`r nrow(test)`. Although glmnet standardises data by default, coefficients are returned on their original scales. To get standardised coefficients for applying to other data, I'm scaling the raw data before inputting to glmnet. 
...
depRS_train = data.frame(
  src_subject_id = train$src_subject_id,
  cbcl_dep = dep,
...
depRS_test = data.frame(
  src_subject_id = test$src_subject_id,
...
#saveRDS(envRS, "G://users/eileen/ABCD/ABCD_Environmental_Risk/ABCDv5.1/DATA/envRS_y2.rds")
...
#write.csv(y2_coefs, "G://users/eileen/ABCD/ABCD_Environmental_Risk/ABCDv5.1/OUTPUT/y2_coefs.csv")
...
y2_coefs_sig |> arrange(desc(beta.min)) |> kbl() |> kable_styling() |> scroll_box()
...
```{r fig.height=5, fig.width=10}
y2_coefs_sig = y2_coefs %>% filter(abs(round(beta.1se, 3))>=0.01)
...
dat = rbind(train, test) |> select(-src_subject_id, -race_ethnicity) |> data.matrix()
...
```{r fig.width=10, fig.height=10}
dat = train |> select(-src_subject_id, -race_ethnicity) |> data.matrix()

cor(dat) |> corrplot::corrplot.mixed(sig.level = 0.05, upper = "color", lower = "number", order = "alphabet", tl.pos = "lt", tl.srt = 45, tl.cex=0.7, lower.col = "black", number.cex = 0.5, diag = "u") +
  theme(panel.grid = element_blank(), panel.background = element_blank())
```
...
```{r fig.height=10, fig.width=10}
dat = test |> select(-src_subject_id, -race_ethnicity) |> data.matrix()
...
rbind(train[,-1], test[,-1]) |> summarytools::dfSummary(round.digits = 3, valid.col = F, varnumbers = F, 
...
# Sensitivity analyses using birthsex instead of gender {.tabset}
...
```{r}
data.frame(Criteria = c("min", "1se"),
           Lambda = c(depfit$lambda.min, depfit$lambda.1se),
           
           MSE = c(depfit$cvm[which(depfit$lambda == depfit$lambda.min)],
                   depfit$cvm[which(depfit$lambda == depfit$lambda.1se)]),
           
           SE = c(depfit$cvsd[which(depfit$lambda == depfit$lambda.min)],
                  depfit$cvsd[which(depfit$lambda == depfit$lambda.1se)]),
           
           RMSE = c(sqrt(depfit$cvm[which(depfit$lambda == depfit$lambda.min)]),
                    sqrt(depfit$cvm[which(depfit$lambda == depfit$lambda.1se)])),
           
           Nonzero = c(sum(coef_dep$beta.min != 0), sum(coef_dep$beta.1se != 0 ))) |>
...
```{r}
data.frame(Criteria = c("min", "1se"),
           Lambda = c(intfit$lambda.min, intfit$lambda.1se),
           
           MSE = c(intfit$cvm[which(intfit$lambda == intfit$lambda.min)],
                   intfit$cvm[which(intfit$lambda == intfit$lambda.1se)]),
           
           SE = c(intfit$cvsd[which(intfit$lambda == intfit$lambda.min)],
                  intfit$cvsd[which(intfit$lambda == intfit$lambda.1se)]),
           
           RMSE = c(sqrt(intfit$cvm[which(intfit$lambda == intfit$lambda.min)]),
                    sqrt(intfit$cvm[which(intfit$lambda == intfit$lambda.1se)])),
...
ggplot(intRS_dat, aes(y = cbcl_int, x = predicted, colour = lambda)) +
  ...
  theme_bw() +
  facet_wrap(~data) +
  labs(x = "predicted intRS", y = "CBCL internalising \n (scaled and centred)")
```
...
y2_coefs_sig |> arrange(desc(beta.1se)) |> kbl() |> kable_styling() |> scroll_box()
