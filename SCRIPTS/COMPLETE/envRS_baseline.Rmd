...
```{r}
base_coefs_sig |> filter(out=="CBCL_depression") |> arrange(desc(beta.min)) |> kbl() |> kable_styling() |> scroll_box(height = "300px")
```

### CBCL internalising

```{r}
base_coefs_sig |> filter(out=="CBCL_internalising") |> arrange(desc(beta.min)) |> kbl() |> kable_styling() |> scroll_box(height = "300px")
```
